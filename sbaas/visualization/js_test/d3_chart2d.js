d3_chart2d = function () {
    // generic chart
    d3_svg.call(this);
    this.duration = 1;
    this.x1scale = null;
    this.y1scale = null;
    this.x2scale = null;
    this.y2scale = null;
    this.colorscale = null;
    this.data1 = {};
    this.data2 = {};
};
d3_chart2d.prototype = Object.create(d3_svg.prototype);
d3_machart2d.prototype.constructor = d3_chart2d;
d3_chart2d.prototype.set_id = function () {
    // set chart id
};
d3_chart2d.prototype.set_svgelement = function () {
    // add svg element
};
d3_chart2d.prototype.add_title = function () {
    // add chart title
};
d3_chart2d.prototype.set_margin = function () {
    // set margin properties
};
d3_chart2d.prototype.set_width = function () {
    // set width properties
};
d3_chart2d.prototype.set_height = function () {
    // set height properties
};
d3_chart2d.prototype.set_x1range = function () {
    // set x1-range of the plot
};
d3_chart2d.prototype.set_y1range = function () {
    // set y1-range of the plot
};
d3_chart2d.prototype.set_x1domain = function () {
    // set x1-domain of the plot
};
d3_chart2d.prototype.set_y1domain = function () {
    // set y1-domain of the plot
};
d3_chart2d.prototype.set_x2range = function () {
    // set x2-range of the plot
};
d3_chart2d.prototype.set_y2range = function () {
    // set y2-range of the plot
};
d3_chart2d.prototype.set_x2domain = function () {
    // set x2-domain of the plot
};
d3_chart2d.prototype.set_y2domain = function () {
    // set y2-domain of the plot
};
d3_chart2d.prototype.set_x1scale = function () {
    // set x1 scale
};
d3_chart2d.prototype.set_x2scale = function () {
    // set x2 scale
};
d3_chart2d.prototype.set_y1scale = function () {
    // set y1 scale
};
d3_chart2d.prototype.set_y2scale = function () {
    // set y2 scale
};
d3_chart2d.prototype.set_colorscale = function () {
    // set color scale
};
d3_chart2d.prototype.add_x1axis = function () {
    //x1 axis properties
};
d3_chart2d.prototype.add_x2axis = function () {
    //x2 axis properties
};
d3_chart2d.prototype.add_y1axis = function () {
    //y1 axis properties
};
d3_chart2d.prototype.add_y2axis = function () {
    //y2 axis properties
};
d3_chart2d.prototype.add_x1labels = function () {
    //x1 lables properties
};
d3_chart2d.prototype.add_x2labels = function () {
    //x2 lables properties
};
d3_chart2d.prototype.add_y1labels = function () {
    //y1 lables properties
};
d3_chart2d.prototype.add_y2labels = function () {
    //y2 lables properties
};
d3_chart2d.prototype.add_xgridlines = function () {
    //x axis grid lines properties
};
d3_chart2d.prototype.add_ygridlines = function () {
    //y axis grid lines properties
};
d3_chart2d.prototype.add_x1title = function () {
    //x1 lables properties
};
d3_chart2d.prototype.add_x2title = function () {
    //x2 lables properties
};
d3_chart2d.prototype.add_y1title = function () {
    //y1 lables properties
};
d3_chart2d.prototype.add_y2title = function () {
    //y2 lables properties
};
d3_chart2d.prototype.add_tooltip = function () {
    //tooltip properties
};
d3_chart2d.prototype.add_legend = function () {
    //legend properties
};
d3_chart2d.prototype.update = function () {
    //update the chart
};
d3_chart2d.prototype.draw = function () {
    //draw the chart
};
d3_chart2d.prototype.filter_data = function () {
    //filter chart data
}
d3_chart2d.prototype.make_lines = function () {
    //make lines
};
d3_chart2d.prototype.add_lines = function () {
    //add lines to chart
};
d3_chart2d.prototype.add_points = function () {
    //points properties
};
d3_chart2d.prototype.add_verticalbars = function () {
    //add vertical bars to the plot
};
d3_chart2d.prototype.add_verticalerrorbars = function () {
    //add vertical error bars to the plot
};
d3_chart2d.prototype.add_horizontalbars = function () {
    //add horizontal bars to the plot
};
d3_chart2d.prototype.add_horizontalerrorbars = function () {
    //add horizontal error bars to the plot
};
d3_chart2d.prototype.add_boxandwhiskers = function () {
    //add box and whiskers to the plot
};
d3_chart2d.prototype.add_heatmap = function () {
    //add heatmap to the plot
};
d3_chart2d.prototype.add_svgexport = function () {
    //add svg element export
};
d3_chart2d.prototype.add_data1 = function () {
    //add data1 element export
};
d3_chart2d.prototype.add_data2 = function () {
    //add data2 element export
};